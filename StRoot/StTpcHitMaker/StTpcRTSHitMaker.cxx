...
Int_t StTpcRTSHitMaker::InitRun(Int_t runnumber) {
  ...
  /*
    ...
    will be set to 1.0!
  */
  if (NoRows <= 45) { // hack for now take T // END OF LOOP OVER SECTORSonko's defaults for iTpx
...
Int_t StTpcRTSHitMaker::Make()
	{
	cout << " -=- StTpcRTSHitMaker IS RUNNING -=- " << endl;
	gBenchmark->Reset();
	gBenchmark->Start("Irakli_CF");
...
//						}
	...
	*/
				if ( dta->sim_cld[i].cld.flags && (dta->sim_cld[i].cld.flags & ~(FCF_ONEPAD | FCF_MERGED | FCF_BIG_CHARGE))) continue;
				if (! hitCollection )
					{
...
//				Update pixels if any (for data)
				...
				while(dta && dta->iterate())
					...
					for(UInt_t i=0;i<dta->ncontent;i++)
						...
						if (ADCs[tb] != dta->sim_adc[i].adc || IDTs[tb] != dta->sim_adc[i].track_id)
							{
							if (Debug() > 1)
								{
		...
		if (maxHits[sec-1] && hitsAdded > maxHits[sec-1])
			{
			LOG_ERROR << "Too many hits (" << hitsAdded << ") in one sector (" << sec << "). Skipping event." << endm;
			cout << "Too many hits (" << hitsAdded << ") in one sector (" << sec << "). Skipping event." << endl;
			return kStSkip;
			}
		} // END OF LOOP OVER SECTORS
	cout << " ********** -=- TIME INFO -=- *********" << endl;
	gBenchmark->Show("Irakli_CF");
	if (maxBin0Hits && bin0Hits > maxBin0Hits)
		{
		LOG_ERROR << "Too many hits (" << bin0Hits << ") starting at time bin 0. Skipping event." << endm;
		return kStSkip;
		}
